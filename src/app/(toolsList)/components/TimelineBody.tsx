...
import { TimelineEvent, ToolsDataEx } from "../types/types";
  ...
  return (
    <>
      {equipment.map((item) => {
        ...
        return (
          <div key={item.asset} className="flex items-stretch mb-0.5">
            ...
            >
              <span
                title={String(item.assetDescription)}
                className="font-medium text-sm  truncate w-full whitespace-nowrap overflow-hidden"
              >
                ...
                <div className="relative flex h-8 rounded ">
                  {days.map((date, dayIndex) => {
                    ...
                    return (
                      <div
                        key={dayIndex}
                        className={`flex-1 h-full border-l border-gray-200 ${bg} border border-gray-200  border-solid rounded`}
                        style={{
                          backgroundColor: `${Obg}`,
                          // border: "solid",
                          borderWidth: "1px",
                ...
                <div key={trackIndex} className="relative flex h-8 rounded">
                  {days.map((date, dayIndex) => {
                    ...
                    return (
                      <div
                        key={dayIndex}
                        className={`flex-1 h-full border-l border-gray-200 ${bg}`}
                        style={{
                          backgroundColor: `${Obg}`,
                          // border: "solid",
                          borderWidth: "1px",
                          ...
                          {Array.from(
                            ...
                            (_, offset) => {
                              ...
                              return (
                                <div
                                  key={offset}
                                  className="absolute h-full bg-red-500 text-white text-xs flex items-center justify-center border border-gray-200  border-solid rounded"
                                  style={{
                                    left: `${leftPercent}%`,
                                    width: `${widthPercent}%`,
                                    backgroundColor: `${ev.color}`,
