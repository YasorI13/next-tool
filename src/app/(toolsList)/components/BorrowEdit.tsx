...
import { BorrowFormEx, pplEx, selectItem } from "../types/types";
...
function BorrowEdit(Props: Props) {
  ...
  const { data : borrowFormData, isLoading : borrowFormLoading, } = useSWR<BorrowFormEx>(
    `emdtools_v1/getBorrowData/${Props.id}`,
    fetcher
  );
  ...
  useEffect(() => {
    if (borrowFormData) {
      if(borrowFormData){
        ...
        setPpl(String(borrowFormData.ppl_id));
        ...
        if(borrowFormData.mmyyyy){
          const mouth = Number(borrowFormData.mmyyyy.split("-")[0]);
          const year = Number(borrowFormData.mmyyyy.split("-")[1]);
          setSelectedMonth(Number(mouth - 1));
          setSelectedYear(year);
        }
        setId_no(borrowFormData.id_no);
      }
    }
